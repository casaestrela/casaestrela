...
class Payment(models.Model):
    ...
    ad_line_total = fields.Monetary(string='Line Total', compute='_compute_ad_line_total')
    ...
    def create(self, values):
        ...
        for rec in self:
            ...
            if rec.ad_line_ids:
                ...
                if ad_lines_total > round(rec.amount,2) or ad_lines_total < round(rec.amount,2):
                    raise ValidationError("Allocation amount cannot be more or less then Payment amount.")
    ...
    def write(self, values):
        ...
        for rec in self:
            ...
            if rec.ad_line_ids:
                for line in rec.ad_line_ids:
                    ad_lines_total += line.reconcile_amount
                if round(ad_lines_total,2) > round(rec.amount,2) or round(ad_lines_total,2) < round(rec.amount,2):
                    raise ValidationError("Allocation amount cannot be more or less then Amount.")
        return res
