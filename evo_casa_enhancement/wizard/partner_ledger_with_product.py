        ...
        for partner in partner_list:
            if partner.name:
                            ...
                            if move_line.account_id in accounts:   
                                ...
                                if move_line.operating_unit_id in operating_unit_list:
                                    ...
                                    if move_line.matching_number == False:  
                                        ...
                                        sheet.write_merge(row,row,3,3,'',cell_text_formate)
                                        ...
                                        sheet.write_merge(row,row,7,7,str(move_line.debit),cell_text_formate)
                                        sheet.write_merge(row,row,8,8,str(move_line.credit),cell_text_formate)
                                        sheet.write_merge(row,row,9,9,str(balance),cell_text_formate)
                                        ...
                                        if move.journal_id.type == 'sale' or move.journal_id.type == 'purchase':
                                            ...
                                            for product_line in move.invoice_line_ids:
                                                ...
                                                sheet.write_merge(row,row,3,3,str(product_line.name),cell_text_formate)
                                                sheet.write_merge(row,row,4,4,str(product_line.quantity),cell_text_formate)
                                                sheet.write_merge(row,row,5,5,str(product_line.product_uom_id.name),cell_text_formate)
                                                sheet.write_merge(row,row,6,6,str(product_line.price_unit),cell_text_formate)
                ...
                if total_partner_debit or total_partner_credit :
                    sheet.write_merge(partner_row,partner_row,0,0,str(partner.name),header)            
                    ...
                    sheet.write_merge(header_row,header_row,5,5,'Unit Of Measure',header)                  
                    sheet.write_merge(header_row,header_row,6,6,'Unit Price',header)          
                    ...
                    sheet.write_merge(partner_row,partner_row,7,7,str(total_partner_debit),header)
                    sheet.write_merge(partner_row,partner_row,8,8,str(total_partner_credit),header)
                    sheet.write_merge(partner_row,partner_row,9,9,str(total_partner_debit - total_partner_credit),header)
                # row += 1
                    row += 1
