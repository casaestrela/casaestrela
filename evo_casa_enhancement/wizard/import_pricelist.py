...
class UpdateProductPriceList(models.TransientModel):
    ...
    def update_product_pricelist(self):
        ...
        if ext[1:] not in ["xls", "xlsx"]:
            raise Warning(
                _(
                    'Unsupported file format "{}", import only supports XLS, XLSX'
        ...
        if result:
            process_log_id = pricelist_update_log.sudo().create(
                        ...
                        pricelist_update_log_details.sudo().create(
                            {
                                ...
                                "file_process_error": _(
                                    "No product found with SLC Code : %s"
                                    % (default_code)
                                ),
                            }
                        )
