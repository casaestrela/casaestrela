...
class DetailProductReport(models.TransientModel):
    ...
    allow_operating_unit_ids = fields.Many2many('operating.unit',readonly=False,string='Allow Operating Unit',store=True,compute='compute_allow_operating_units')
    user_id = fields.Many2one('res.users',default=lambda self: self.env.user)
    operating_unit = fields.Many2one('operating.unit',domain="[('id','in',allow_operating_unit_ids)]")
    ...
    def action_confirm_report(self):
        ...
        header = workbook.add_format({'bold':True, 'valign': 'center','bg_color':'gray'})
        ...
        row += 1
        if not self.env.user.has_group("evo_casa_enhancement.group_inventory_adjustment"):
            ...
            row += 1
            location_id = self.env['stock.location'].sudo().search([('operating_unit_id','=',self.operating_unit.id)],limit=1)
            QTY_INCOMING = self.product_id.with_context({'to_date':  self.start_date,'location' : location_id.id}).incoming_qty
            QTY_OUTGOING = self.product_id.with_context({'to_date':  self.start_date,'location' : location_id.id}).outgoing_qty                            
            ...
            TOTAL_CLOSE_QTY = 0.0
            product_move = self.env['stock.move'].sudo().search([
            ...
            for product in product_move:       
                product_date = datetime.strptime(str(product.date), "%Y-%m-%d %H:%M:%S").date()
                    ...
                    if product.picking_id.picking_type_code:
                        ...
                        if product.picking_id.picking_type_code == 'internal':  
                            ...
                            if product.picking_id.location_id == location_id:
                                OUTWARD_QTY = product.product_uom_qty
                                INWARD_QTY = 0.0
                                particular = product.picking_id.location_id.operating_unit_id.name 
                            if product.picking_id.location_dest_id == location_id:
                                INWARD_QTY = product.product_uom_qty
                                OUTWARD_QTY = 0.0
                                particular = product.picking_id.location_dest_id.operating_unit_id.name            
        ...
        if self.env.user.has_group("evo_casa_enhancement.group_inventory_adjustment"):
            ...
            for product in product_move:       
                ...
                if product.picking_id or product.inventory_id:
                    if product.picking_id.picking_type_code:
                        ...
                        if product.picking_id.picking_type_code == 'incoming':  
                            ...
                            if product.purchase_line_id.price_unit > 0.0:
                                OPENING_INWARD_PRICE = product.purchase_line_id.price_unit
                    ...
                    sheet.write(row, 12, str(CLOSE_QTY * OPENING_INWARD_PRICE),cell_text_formate)
                    INWARD_QTY = 0.0
                    OUTWARD_QTY = 0.0
                    INWARD_PRICE = 0.0
                    OUTWARD_PRICE = 0.0
            ...
            sheet.write(OPENING_ROW, 6, str(OPENING_INWARD_PRICE * OPENING_BALANCE_QTY),cell_text_formate)  
            sheet.write(OPENING_ROW, 11,str(OPENING_INWARD_PRICE),cell_text_formate)             
            sheet.write(OPENING_ROW, 12,str(OPENING_INWARD_PRICE * OPENING_BALANCE_QTY),cell_text_formate)
        ...
        return {
            ...
            'url': '/web/content/?model=detail.product.report&field=excel_detail_report&id=%s&filename=%s&download=true' % (self.id,file_name),
            'target': 'self',
            }        
