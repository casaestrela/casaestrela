...
class AccountMove(models.Model):
    ...
    def action_post(self):
        if self.move_type == 'out_refund':
            sale_order_id = self.env['sale.order'].sudo().search([('name','=',self.invoice_origin)],limit=1)
            self.sale_order_id = sale_order_id.id
            picking_id = self.env['stock.picking'].sudo().search([('sale_id','=',sale_order_id.id)],limit=1)
    ...
    def action_sale_return(self):
        ...
        wizard_id = self.env['account.move.reversal'].sudo().create(
                    {
...
class AccountMoveLine(models.Model):
    ...
    analytic_account_id = fields.Many2one('account.analytic.account', string='Analytic Account',default=_default_analytic_account_id,
    ...
    margin = fields.Float("Margin", compute='_compute_margin',digits='Product Price', store=True, groups="base.group_user")
