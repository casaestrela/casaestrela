...
class PoSPaymentReportWizard(models.TransientModel):
    ...
    operating_unit_id = fields.Many2one('operating.unit',string="Operating Unit", default=lambda self: (self.env["res.users"].operating_unit_default_get(self.env.uid)))
    ...
    def generate_pos_payment_report(self):
        ...
        bold_number_formate = workbook.add_format({'bold':True,'num_format': '#,##0.00'})
        
        #set column row width
        sheet.set_column(0, 10, 15)
        
        # File Content
        tital_name = 'Payment Report : ' + self.date_from.strftime("%d-%b-%Y") + " to " + self.date_to.strftime("%d-%b-%Y")
        dates = self.date_range(self.date_from, self.date_to)
        
        pos_payment_methods_ids = self.env['pos.payment.method'].search([])
        account_journal_ids = self.env['account.journal'].search([('type','in',('bank','cash'))])
        sheet.merge_range(0, 0, 0, 1 + len(pos_payment_methods_ids) + len(account_journal_ids) or 5, tital_name or '', bold)
        ...
        for payment_method_id in pos_payment_methods_ids:
            for date in dates:
                total_amount = 0.0
                pos_payment_ids = self.env['pos.payment'].search([('payment_date','>=',date.strftime("%Y-%m-%d 00:00:00")),
                                                                  ('payment_date','<=',date.strftime("%Y-%m-%d 23:59:59")),
                                                                  ('payment_method_id','=',payment_method_id.id),
                                                                  ('pos_order_id.operating_unit_id','=',self.operating_unit_id.id)])
        ...
        for account_journal_id in account_journal_ids:
            for date in dates:
                total_amount = 0.0
                account_payment_ids = self.env['account.payment'].search([('date','>=',date.strftime("%Y-%m-%d")),
        ...
        for date,type_data in payment_dic.items():
            ...
            for payment_method_id in pos_payment_methods_ids:
                ...
                if payment_method_id in payment_amount_dic:
                    payment_amount_dic[payment_method_id] += type_data[payment_method_id]
                else:
                    payment_amount_dic.update({payment_method_id:type_data[payment_method_id]})
                col += 1
            ...
            for account_journal_id in account_journal_ids:
                ...
                if account_journal_id in payment_amount_dic:
                    payment_amount_dic[account_journal_id] += type_data[account_journal_id]
                else:
                    payment_amount_dic.update({account_journal_id:type_data[account_journal_id]})
                col += 1
            sheet.write(row, col, total_pos_row_amount + total_journal_row_amount, bold_number_formate)
            
            if 'amount' in payment_amount_dic:
                payment_amount_dic['amount'] += total_pos_row_amount + total_journal_row_amount
            else:
                payment_amount_dic.update({'amount':total_pos_row_amount + total_journal_row_amount})
        ...
        col = 1
        for payment_method_id in pos_payment_methods_ids:
            sheet.write(row, col, payment_amount_dic[payment_method_id], bold_number_formate)
            col += 1
        for account_journal_id in account_journal_ids:
            sheet.write(row, col, payment_amount_dic[account_journal_id], bold_number_formate)
            col += 1
        ...
        return {
            ...
            'url': '/web/content/?model=pos.payment.report.wizard&field=excel_detail_report&id=%s&filename=%s&download=true' % (self.id,file_name),
            'target': 'self',
            }
