            ...
            if order.get('data').get('statement_ids'):
                ...
                if pos_order_id.payment_ids:
                    for line in pos_order_id.payment_ids:
                        line.cheque_number = order.get('data').get('statement_ids')[0][2]['cheque_number']
                        line.cheque_date = order.get('data').get('statement_ids')[0][2]['cheque_date']
...
class PosOrderLine(models.Model):
    ...
    def get_msp_subtotal(self):
        for record in self:
            ...
            record.msp_subtotal = record.price_subtotal_incl - ((record.price_subtotal_incl * record.msp_percentage) / 100)
