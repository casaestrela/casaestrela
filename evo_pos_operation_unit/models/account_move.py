    ...
    def create(self, values):
        ...
        if res.pos_session_id:
            if res.pos_session_id.config_id:
                res.operating_unit_id = res.pos_session_id.config_id.operating_unit_id.id
        return res
    ...
    def create(self, values):
        ...
        for data in res:
            if data.operating_unit_id:
                analytic_account_id = self.env['account.analytic.account'].sudo().search([('operating_unit_ids','=',data.operating_unit_id.id)],limit=1)
