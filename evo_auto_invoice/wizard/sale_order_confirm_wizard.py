...
class SaleOrderConfirmWizard(models.TransientModel):
    ...
    def _compute_currency_id(self):
        for wizard in self:
            wizard.currency_id = (
                wizard.payment_method_id.currency_id or wizard.company_id.currency_id
            )
        ...
        for wizard in self:
                    ...
                    .create(
                        {
                            ...
                            and wizard.payment_method_id.inbound_payment_method_ids
                            and wizard.payment_method_id.inbound_payment_method_ids[
                                0
                            ].id
                            or False,
            ...
            for record in wizard.sale_order_id.invoice_ids:
                journal_move_id = (
                    ...
                    .sudo()
                    .search([("ref", "=", record.name)], limit=1)
                )
                payment_id = (
                    self.env["account.payment"]
                    .sudo()
                    .search([("move_id", "=", journal_move_id.id)], limit=1)
                )
