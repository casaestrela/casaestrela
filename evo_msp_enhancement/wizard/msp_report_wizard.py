...
class MSPReportWizard(models.TransientModel):
    ...
    def action_generate_report(self):
        ...
        header = xlwt.easyxf('font:bold True;border:left thin,right thin,top thin, bottom thin;align:horiz center;')
        ...
        row = 1
        total = 0.0
        invoice_ids = self.env['account.move'].sudo().search([('move_type','=','out_invoice'), ('state', '=', 'posted'),('operating_unit_id','=',self.operating_unit_id.id)],order='invoice_date asc')#('payment_state', '=', 'paid'),
        for invoice in invoice_ids:
            ...
            sheet.write_merge(row,row,2,2,str(round(total_msp,2)),cell_text_formate)
            row += 1
        ...
        action = {
            ...
            'name':'Excel Report',
            'url': '/web/content/?model=msp.report.wizard&field=excel_detail_report&id=%s&filename=%s&download=true' % (self.id,file_name),
            'target': 'new',
        }
        return action
