...
class AccountReconciliation(models.AbstractModel):
    ...
    def get_move_lines_for_bank_statement_line(
        ...
        aml_accounts = [st_line.journal_id.default_account_id.id]
    ...
    def _get_bank_statement_line_partners(self, st_lines):
        ...
        """
        query += (
            "LEFT JOIN res_partner_bank bank ON bank.id = move.partner_bank_id OR bank.sanitized_acc_number ILIKE regexp_replace(st_line.account_number, '\\W+', '', 'g') %s\n"
            % (where_bank)
        )
        ...
        query += (
            "LEFT JOIN res_partner p3 ON p3.name ILIKE st_line.partner_name %s AND p3.parent_id is NULL \n"
            % (where_partner)
        )
    ...
    def get_data_for_manual_reconciliation(
        ...
        """.format(
            select=is_partner
            and "partner_id, partner_name, to_char(last_time_entries_checked, 'YYYY-MM-DD') AS last_time_entries_checked,"
            or " ",
            inner_select=is_partner
            and "p.id AS partner_id, p.name AS partner_name, p.last_time_entries_checked AS last_time_entries_checked,"
            or " ",
            ...
            where1=is_partner
            and " "
            or "AND ((at.type <> 'payable' AND at.type <> 'receivable') OR l.partner_id IS NULL)",
            ...
            and "WHERE (last_time_entries_checked IS NULL OR max_date > last_time_entries_checked)"
            or " ",
        )
        self.env["account.move.line"].flush()
