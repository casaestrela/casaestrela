...
class AgeingView(models.TransientModel):
    ...
    def get_filter_data(self, option):
        ...
        categories = (
            r.partner_category_ids
            if r.partner_category_ids
    ...
    def _get_report_values(self, data):
        ...
        account_res = self._get_partner_move_lines(
            data, partners, date_from, target_move, account_type, data["period_length"]
        )
    ...
    def _get_partner_move_lines(
        self, data, partners, date_from, target_move, account_type, period_length
    ):
        ...
        for i in range(5):
            ...
            for line in self.env["account.move.line"].browse(aml_ids):
                ...
                if (
                    not self.env.company.currency_id.is_zero(line_amount)
                    and partner_id != False
                ):
    ...
    def get_dynamic_xlsx_report(self, data, response, report_data, dfr_data):
        ...
        sheet.merge_range(
            "A2:H3", filters.get("company_name") + ":" + " Partner Ageing", head
        )
        ...
        if filters.get("date_from"):
            sheet.merge_range(
                "A4:B4", "As On Date: " + filters.get("date_from"), date_head
            )
        sheet.merge_range(
            "C4:E4", "Account Type: " + filters.get("result_selection"), date_head
        )
        sheet.merge_range(
            "A5:B5", "Target Moves: " + filters.get("target_move"), date_head
        )
        sheet.merge_range(
            "D5:F5",
        ...
        sheet.merge_range(
            "G5:H5",
            " Partner Type: " + ", ".join([lt or "" for lt in filters["partner_tags"]]),
            date_head,
        )
