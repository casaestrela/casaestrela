...
class AgeingView(models.TransientModel):
    ...
    def _get_report_values(self, data=None):
        ...
        journals = (
            self.env["account.journal"].search([("id", "in", active_jrnl)])
    ...
    def _get_account_move_entry(self, accounts, form_data, journals, pass_date):
        ...
        params = (tuple(accounts.ids), tuple(journals.ids), pass_date)
    ...
    def get_dynamic_xlsx_report(self, data, response, report_data, dfr_data):
        ...
        sheet.merge_range(
            "A2:D3", filters.get("company_name") + ":" + " Day Book", head
        )
        ...
        sheet.merge_range(
            "E4:F4", "Target Moves: " + filters.get("target_move"), date_head
        )
        sheet.merge_range(
            "B5:D5",
            "Account Type: " + ", ".join([lt or "" for lt in filters["accounts"]]),
            date_head,
        )
