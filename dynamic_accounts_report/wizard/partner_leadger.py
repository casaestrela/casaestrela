...
class PartnerView(models.TransientModel):
    ...
    def get_filter(self, option):
        ...
        if data.get("partners"):
            filters["partners"] = (
                self.env["res.partner"].browse(data.get("partners")).mapped("name")
            )
        else:
    ...
    def get_filter_data(self, option):
        ...
        partner = r.partner_ids if r.partner_ids else self.env["res.partner"].search([])
        categories = (
    ...
    def _get_report_values(self, data):
        ...
        partner_res = self._get_partners(
            partners, accounts, init_balance, display_account, data
        )
            ...
            vals.update(
                {
                    "partner_category_ids": [
                        (4, j) for j in vals.get("partner_category_ids")
                    ]
                }
            )
    ...
    def _get_partners(self, partners, accounts, init_balance, display_account, data):
        ...
        if data.get("reconciled") == "unreconciled":
            WHERE += (
                " AND l.full_reconcile_id is null AND"
                " l.balance != 0 AND a.reconcile is true"
            )

        sql = (
            """SELECT l.id AS lid,l.partner_id AS partner_id,m.id AS move_id,
        ...
        for row in cr.dictfetchall():
            ...
            if row["partner_id"] in move_lines:
                ...
                row["account_name"] = (
                    account_list[row["account_id"]]["name"]
                    + "("
                    + account_list[row["account_id"]]["code"]
                    + ")"
                )
    ...
    def get_dynamic_xlsx_report(self, data, response, report_data, dfr_data):
        ...
        cell_format = workbook.add_format(
            {"align": "center", "bold": True, "border": 0}
        )
        ...
        sheet.merge_range(
            "A1:H2", filters.get("company_name") + ":" + "Partner Ledger", head
        )
        ...
        sheet.merge_range(
            "A4:B4", "Target Moves: " + filters.get("target_move"), date_head
        )

        sheet.merge_range(
            "C4:D4",
            "Account Type: " + ", ".join([lt or "" for lt in filters["account_type"]]),
            date_head,
        )
        sheet.merge_range(
            "E3:F3",
            " Partners: " + ", ".join([lt or "" for lt in filters["partners"]]),
            date_head,
        )
        sheet.merge_range(
            "G3:H3",
            " Partner Tags: " + ", ".join([lt or "" for lt in filters["partner_tags"]]),
            date_head,
        )
        sheet.merge_range(
            "A3:B3",
            " Journals: " + ", ".join([lt or "" for lt in filters["journals"]]),
            date_head,
        )
        sheet.merge_range(
            "C3:D3",
            " Accounts: " + ", ".join([lt or "" for lt in filters["accounts"]]),
            date_head,
        )
        ...
        for report in report_data:
            ...
            row += 1
            sheet.merge_range(
                row, col + 0, row, col + 4, report["name"], sub_heading_sub
            )
