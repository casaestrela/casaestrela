...
class AccountCasgFlow(models.TransientModel):
    ...
    def _get_report_values(self, data, option):
        ...
        if data.get("levels") == "summary":
            ...
            query3 = (
                """SELECT to_char(am.date, 'Month') as month_part, extract(YEAR from am.date) as year_part,
            ...
            sql = (
                """SELECT DISTINCT aa.id, aa.name,aa.code, sum(aml.debit) AS total_debit,
            ...
            query2 = (
                """SELECT aat.name, sum(aml.debit) AS total_debit, sum(aml.credit) AS total_credit,
            ...
            query1 = (
                """SELECT aa.id,aa.name,aa.code, sum(aml.debit) AS total_debit, sum(aml.credit) AS total_credit,
                         sum(aml.balance) AS total_balance FROM (SELECT am.id, am.state FROM account_move as am
                         LEFT JOIN account_move_line aml ON aml.move_id = am.id
                         LEFT JOIN account_account aa ON aa.id = aml.account_id
                         LEFT JOIN account_account_type aat ON aat.id = aa.user_type_id
    ...
    def _get_lines(self, account, data):
        ...
        query = (
            """SELECT aml.account_id,aj.id as j_id,aj.name,am.id, am.name as move_name, sum(aml.debit) AS total_debit,
        ...
        sql2 = (
            """SELECT aa.name as account_name,aa.id as account_id, aj.id, aj.name, sum(aml.debit) AS total_debit,
    ...
    def get_journal_lines(self, account, data, offset=0, fetch_range=FETCH_RANGE):
        ...
        sql2 = (
            """SELECT aa.name as account_name, aj.name, sum(aml.debit) AS total_debit,
         sum(aml.credit) AS total_credit, COALESCE(SUM(aml.debit - aml.credit),0) AS balance FROM (SELECT am.* FROM account_move as am
             LEFT JOIN account_move_line aml ON aml.move_id = am.id
             LEFT JOIN account_account aa ON aa.id = aml.account_id
             LEFT JOIN account_account_type aat ON aat.id = aa.user_type_id
    ...
    def get_dynamic_xlsx_report(self, data, response, report_data, dfr_data):
        ...
        cell_format = workbook.add_format({"bold": True, "font_size": "10px"})
        ...
        txt = workbook.add_format({"align": "left", "font_size": "10px"})
        txt_left = workbook.add_format(
            {"align": "left", "font_size": "10px", "border": 1}
        )
        txt_center = workbook.add_format(
            {"align": "center", "font_size": "10px", "border": 1}
        )
        amount = workbook.add_format(
            {"align": "right", "font_size": "10px", "border": 1}
        )
        amount_bold = workbook.add_format(
            {"align": "right", "bold": True, "font_size": "10px", "border": 1}
        )
        txt_bold = workbook.add_format(
            {"align": "left", "bold": True, "font_size": "10px", "border": 1}
        )
        ...
        for i_rec in fetched_data_list:
            if data["levels"] == "summary":
                sheet.write(
                    row_num + 1,
                    col_num,
                    str(i_rec["month_part"]) + str(int(i_rec["year_part"])),
                    txt_left,
                )
                sheet.write(
                    row_num + 1,
                ...
                sheet.write(
                    row_num + 1,
                    col_num + 3,
                    str(i_rec["total_debit"] - i_rec["total_credit"])
                    + str(currency_symbol),
                    amount,
                )
                row_num = row_num + 1
            ...
            if data["levels"] == "detailed":
                for k in fetched_data_list:
                    if k["name"] == j_rec["account"]:
                        ...
                        sheet.write(
                            row_num + 1,
                            col_num + 3,
                            str(k["total_debit"] - k["total_credit"])
                            + str(currency_symbol),
                            amount_bold,
                        )
                        row_num = row_num + 1
                for l_jrec in j_rec["journal_lines"]:
                    ...
                    sheet.write(
                        row_num + 1,
                        col_num + 1,
                        str(l_jrec["total_debit"]) + str(currency_symbol),
                        amount,
                    )
                    sheet.write(
                        row_num + 1,
                        col_num + 2,
                        str(l_jrec["total_credit"]) + str(currency_symbol),
                        amount,
                    )
                    sheet.write(
                        row_num + 1,
                        col_num + 3,
                        str(l_jrec["total_debit"] - l_jrec["total_credit"])
                        + str(currency_symbol),
                        amount,
                    )
                    row_num = row_num + 1
                ...
                for l_jrec in j_rec["journal_lines"]:
                    ...
                    for m_rec in j_rec["move_lines"]:
                        if m_rec["name"] == l_jrec["name"]:
                            sheet.write(
                                row_num + 1, col_num, m_rec["move_name"], txt_center
                            )
                            sheet.write(
                                row_num + 1,
                                col_num + 1,
                                str(m_rec["total_debit"]) + str(currency_symbol),
                                amount,
                            )
                            sheet.write(
                                row_num + 1,
                                col_num + 2,
                                str(m_rec["total_credit"]) + str(currency_symbol),
                                amount,
                            )
                            sheet.write(
                                row_num + 1,
                                col_num + 3,
                                str(m_rec["total_debit"] - m_rec["total_credit"])
                                + str(currency_symbol),
                                amount,
                            )
                            row_num = row_num + 1
