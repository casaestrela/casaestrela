...
class GeneralView(models.TransientModel):
    ...
    def get_filter_data(self, option):
        ...
        analytic_tags = (
            ...
            else self.env["account.analytic.tag"].search([])
        )
        filter_dict = {
    ...
    def write(self, vals):
        ...
        if vals.get("account_tag_ids"):
            vals.update(
                {"account_tag_ids": [(4, j) for j in vals.get("account_tag_ids")]}
            )
        if vals.get("account_tag_ids") == []:
    ...
    def _get_accounts(self, accounts, init_balance, display_account, data):
        ...
        sql = (
            """SELECT l.id AS lid,m.id AS move_id, l.account_id AS account_id, l.date AS ldate, j.code AS lcode, l.currency_id, l.amount_currency, l.ref AS lref, l.name AS lname, COALESCE(l.debit,0) AS debit, COALESCE(l.credit,0) AS credit, COALESCE(SUM(l.balance),0) AS balance,\
    ...
    def get_dynamic_xlsx_report(self, data, response, report_data, dfr_data):
        ...
        sheet.merge_range(
            "A2:J3", filters.get("company_name") + ":" + name_data.get("name"), head
        )
