...
class FundTransferMaster(models.Model):
    ...
    def get_bank_cash_account_ids(self):
        bank_cash_type_id = self.env['account.account.type'].sudo().search([('name', '=', 'Bank and Cash')], limit=1)
        account_ids = self.env['account.account'].sudo().search([('user_type_id', '=', bank_cash_type_id.id)])
        op_unit_id = self.env["res.users"].operating_unit_default_get(self.env.uid)
        account_id_list = []
        if account_ids:
            for rec in account_ids:
                operat_id = self.env['operating.unit'].sudo().search([('account_id', '=', rec.id)])
                if operat_id and rec.id not in account_id_list and op_unit_id.account_id.id != rec.id:
    ...
    date = fields.Date(string='Transaction Date', states={'draft': [('readonly', False)]}, copy=False, default=fields.Date.context_today)
    subtotal = fields.Float("Subtotal", default=0.0)
    reject_lines = fields.One2many('fund.transfer.reject.reason', 'transfer_id', string='Reject Lines',copy=False)
    ...
    def action_verified(self):
        operating_unit_id = self.env["res.users"].operating_unit_default_get(self.env.uid)
        if self.to_operating_unit_id.id != operating_unit_id.id:
            raise UserError(_("Only 'To Operating Unit' user can verified the fund transfer."))

        if self.move_id:
            line_list = []
            line_list.append({'debit': self.subtotal,
                              'account_id': self.to_account_id.id,
                              'move_id': self.move_id.id,
                              'operating_unit_id': self.to_operating_unit_id.id,
                              })
            line_list.append({'credit': self.subtotal,
            ...
            self.move_id.write({
                'line_ids': [(6, 0, move_line_ids.ids)]
            })
        else:
            line_list = []
            line_list.append((0, 0, {'debit': self.subtotal,
                                     'account_id': self.to_account_id.id,
                                     'operating_unit_id': self.to_operating_unit_id.id,
                                     }))
            line_list.append((0, 0, {'credit': self.subtotal,
                                     'account_id': self.from_account_id.id,
                                     'operating_unit_id': self.operating_unit_id.id,
                                     }))
    ...
    def action_validate(self):
        ...
        if self.operating_unit_id.id != operating_unit_id.id:
            raise UserError(_("Only 'From Operating Unit' user can validate the fund transfer."))
    ...
    def _onchange_to_account_id(self):
        if self.to_account_id:
            operat_id = self.env['operating.unit'].sudo().search([('account_id', '=', self.to_account_id.id)], limit=1)
            if operat_id:
