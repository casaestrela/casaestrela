...
class ProductTemplate(models.Model):
    ...
    stock_location = fields.Many2one('stock.location',string='Stock Location',store=True,compute='compute_stock_location_qty')
    stock_location_qty = fields.Float('Stock Location Qty',compute='compute_stock_location_qty')
    ...
    def compute_current_user(self):
        for rec in self:
            ...
            rec.stock_location_qty = rec.with_context({'location' : location_id.id,'company_id':self.env.user.company_id.id}).qty_available
