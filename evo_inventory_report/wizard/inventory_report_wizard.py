...
class InventoryValuationReportWizard(models.TransientModel):
    ...
    def action_confirm_report(self):
        ...
        stock_move_id = self.env['stock.move'].sudo().search([
        ...
        for move in stock_move_id:
            print('------------move------------------',move)
            existing_report_id = self.env['inventory.valuation.report'].sudo().search([('name','=',move.product_id.id)])
            if existing_report_id:
                ...
                QTY_INCOMING = move.product_id.with_context({'to_date':  self.start_date,'location' : self.location.id,'company_id':self.company_id.id}).incoming_qty
                QTY_OUTGOING = move.product_id.with_context({'to_date':  self.start_date,'location' : self.location.id,'company_id':self.company_id.id}).outgoing_qty                            
                BEGINNING_QTY = QTY_INCOMING - QTY_OUTGOING
                report_id = self.env['inventory.valuation.report'].sudo().create({
        ...
        for report in report_object:
            if report.internal > 0:
                report.ending = report.beginning - report.internal + report.received - report.sales + report.adjustment
            if report.internal < 0:
                report.ending = report.beginning + report.internal + report.received - report.sales + report.adjustment
            valuation_id = self.env['stock.valuation.layer'].sudo().search([('stock_move_id','=',report.stock_move_id.id),('product_id','=',report.name.id)],limit=1)
            # report.valuation = report.ending * valuation_id.unit_cost
            report.valuation = report.ending * report.cost_price
        
        tree_view_id = self.env.ref('evo_inventory_report.view_inventory_valuation_report_tree')
        return {
