...
class SaleOrderQuote(models.Model):
    ...
    _sql_constraints = [
        (
            ...
            _(
                "You can only have one internal quote per operating unit and "
                "sale order!"
            ),
        ),
    ]
    ...
    def _onchange_operating_unit_id(self):
        if self.operating_unit_id:
            ...
            self.name = sale_id.name or "New" + " - " + self.operating_unit_id.code
    ...
    def prepare_crm_lead_values(self):
        ...
        return {
            "name": self.sale_id.name + " - " + self.operating_unit_id.code,
