            ...
            if values:
                if i == 0:
                    ...
                    if values['category_id']:
                        category_id = self.env['product.category'].sudo().search([('name','=',values['category_id'])],limit=1)
                        operating_units = self.env['operating.unit'].sudo().search([])
                        if category_id:
                            category_id.category_prefix = values['prefix']
                        else:
                            category_id = self.env['product.category'].sudo().create({
                                                                                        'name':values['category_id'],
                                                                                        'category_prefix':values['prefix'],
                                                                                        'operating_unit_ids':[(4, unit.id) for unit in operating_units],
                                                                                    })
                        pos_category_id = self.env['pos.category'].sudo().search([('name','=',values['category_id'])],limit=1)
                        if not pos_category_id:
                            pos_category_id = self.env['pos.category'].sudo().create({
                                                                                        'name':values['category_id'],
                                                                                    })
                        unit_category_id = self.env['uom.category'].sudo().search([('name','=','Custom '+values['unit'])],limit=1)
                        if not unit_category_id:
                            unit_category_id = self.env['uom.category'].sudo().create({
                                                                                        'name':'Custom '+values['unit'],
                                                                                        })
                        unit_id = self.env['uom.uom'].sudo().search([('name','=',values['unit'])],limit=1)
                        if not unit_id:
                            unit_id = self.env['uom.uom'].sudo().create({
                                                                        'name':values['unit'],
                                                                        'uom_type':'reference',
                                                                        'active':True,
                                                                        'rounding':0.01000,
                        ...
                        inventory_id = self.env['stock.inventory'].sudo().create({
                                                                                    ...
                                                                                    'location_ids':[(4,location_id.id,0)],#location_id.id,
                                                                                    'product_ids':[(4,product_id.id,0)],#product_id.id,
                                                                                    'prefill_counted_quantity':'counted',
                                                                                    })
                        inventory_id.action_start()
                        inventory_line_id = self.env['stock.inventory.line'].sudo().create({
